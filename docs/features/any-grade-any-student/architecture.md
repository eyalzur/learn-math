...
נבנה בדיוק לפי התוכנית, בשני קבצים בלבד: `src/data/curriculum.ts` (הסרת `gradeIds`/
`gradeOf`/`availableGrades`) ו-`src/App.tsx` (כל שאר השינויים המתוארים למעלה — מפתח
אחסון חדש, `resolveScreen`, עדכון שני ה-`useState` הראשוניים, `selectStudent`,
`chooseGrade`, `enterTopic`, ושני ה-`onBack` של `levels`/`styles`). שום קובץ אחר לא
נגע — `StudentPicker.tsx`, `GradePicker.tsx`, `TopicPicker.tsx` כבר עבדו כמו שצריך
בלי שינוי.

**סטייה טכנית קטנה אחת**, לא ברמת ארכיטקטורה: הפישוט של `grades.length > 1 ? X : Y`
מופיע בקוד בפועל בשלוש מקומות, לא שניים כמו שנרשם ב-Affected Files — הבלוק השלישי
(ה-`return` האחרון בסוף הפונקציה, ללא JSX עטיפה מוקדמת) נכתב בהזחה שונה מהבלוק
הראשון ("בית"), אז שני מופעי ה-ternary בו לא נתפסו ב-replace_all הראשון וטופלו
בנפרד. אותו תיקון בדיוק, רק בשני מעברים במקום אחד.

**אימות ידני לפני מסירה ל-QA**: הרצתי סקריפט Playwright זמני (לא נשמר בריפו, `4`
בדיקות) שמאמת ישירות את התרחישים המרכזיים: (1) רותם רואה עכשיו מסך בחירת כיתה עם כל
`4` הכיתות ויכול לבחור כיתה א׳ (לא כיתה ו׳ שלו); (2) כניסה למסך סגנונות ואז `reload()`
חוזרת ישר לאותו מסך סגנונות, בלי לעבור דרך בחירת תלמיד/כיתה; (3) חזרה מסגנונות לרשימת
נושאים מנקה את המיקום העמוק — `reload()` אחרי זה נופלת לרשימת הנושאים, לא לסגנונות;
(4) נושא אדפטיבי (`חיבור עד 10`) לא משמר "בפנים" — `reload()` אחרי כניסה לתרגול חוזרת
לרשימת הנושאים, לא לתוך התרגול. כל `4` עברו.
