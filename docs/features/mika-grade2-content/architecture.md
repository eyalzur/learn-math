...
## Implementation Notes — סבב תיקונים אחרי הריצה הראשונה

הריצה הראשונה של `test:e2e` (אחרי `npm run build`/`lint` נקיים) גילתה `76` בדיקות
קיימות שנשברו — הרבה יותר מהחשש שכבר תועד ב-Edge Cases. שני סוגי תיקון, לא אחד:

**1. תיקוני בדיקה בלבד (הרוב).** כ-15 קובצי בדיקה מהפייצ'רים הקודמים לוחצים על
"מיקה" (התלמידה הראשונה) ומצפים למסך הנושאים מיד אחריה — עכשיו יש מסך בחירת כיתה
ביניהם. תוקן בכל אחד: קליק על `.grade-card` אחרי בחירת מיקה, בדיוק כמו שכל שאר
הפייצ'רים כבר עושים. גם שני מספרים קשיחים ב-`content.spec.ts` (`LEVEL_SIZE`, ומספר
השאלות הכתובות בטור: `30`→`80`) ורמז אחד עם `10` לא מסומן בגרשיים בכיתה ב׳ עצמה.

**2. באג אמיתי באפליקציה, לא רק בבדיקות.** לחיצה על "ההתקדמות שלי" ממסך בחירת הכיתה
לא עבדה בכלל: הניתוב ב-`App.tsx` בדק "האם יש יותר מכיתה אחת ועדיין לא נבחרה" *לפני*
שבדק אם המסך המבוקש הוא היסטוריה — אז הלחיצה תמיד "נבלעה" וחזרה לאותו מסך. תוקן על
ידי הזזת הבדיקה של מסך ההיסטוריה להיות ראשונה, לפני שער בחירת הכיתה — היסטוריה לא
תלויה בכיתה בכלל, אז אין סיבה שהשער יחסום אותה.

**3. תיקון עיצוב אמיתי, לא רק קוד.** גילוי דרך הבדיקות שהוביל לתיקון ב-design.md
עצמו: "ההתקדמות שלי" תוכנן במקור להופיע *רק* במסך בחירת הכיתה (לא כפול במסך
הנושאים) — אבל זה יצר מצב שבו מיקה שכבר בתוך נושא לא יכולה להגיע להיסטוריה שלה בלי
לחזור עד למסך בחירת הכיתה, בזמן שלכל תלמיד/ה אחר/ת זה קליק אחד. תוקן: הקישור מופיע
עכשיו גם וגם. זה חייב גם תיקון ל-`TopicPicker.tsx`: הטקסט של כפתור "חזרה" היה מחושב
מתוך "האם `onHistory` הועבר" כתחליף ל"האם זה המסך הראשון" — הכרעה שנכונה תמיד עד
עכשיו, כי שני הדברים תמיד קרו יחד. מיקה שוברת את ההנחה הזו: יש לה גם קישור היסטוריה
וגם "חזרה" שלא מחליף תלמיד/ה. נוסף `backLabel: string` — פרופ מפורש שהקורא (שכן
יודע מה `onBack` בפועל עושה) מעביר, במקום שהרכיב ינחש.

**4. תיקון ארכיטקטורה אמיתי, לא רק תיקון נקודתי.** design.md המקורי קבע "הבחירה אף
פעם לא נזכרת בין ביקורים", בטענה שזה עקבי עם "מי לומד היום?" — טענה שהתבררה כשגויה:
"מי לומד היום?" כן נשמר דרך רענון דף (`localStorage`), ומפסיק להיזכר רק כשעוזבים
במפורש דרך "החלף תלמיד/ה". נוסף `gradeId`/`rememberGradeId`/`loadGradeId` ב-`App.tsx`
שעוקבים בדיוק אחרי אותו כלל: כיתה נבחרת נשמרת דרך רענון, ומתאפסת רק כש-"← חזרה"
ממסך הנושאים נלחץ במפורש (מה שגם מנקה את מה ששמור, לא רק את הזיכרון החי — כדי שרענון
דף באמצע "אני רוצה לשקול מחדש" לא יקפוץ בחזרה לבחירה הישנה).

כל התיקונים האלה נבדקו יחד בריצה נקייה אחת: **`183`/`183` עוברות.**
