...
## תיקון אחרי משוב על התצוגה המקדימה (`2026-08-19`)
המשתמש, אחרי שראה את השאלה "כמה זה 10% מ-200?" בפועל: "יש שם מרכאות שזה רע ופחות
מידי מספרים. במקום רק לכתוב 1 חלקי 10 לכתוב את השבר עצמו בתור מספר." שני תיקונים
נפרדים, אחד קוד ואחד תוכן:

1. **באג רינדור אמיתי**: `src/components/Practice.tsx` הציג `step.label` כטקסט גולמי
   (`<span>{step.label}</span>`) בלי להעביר אותו דרך `segmented()` — הפונקציה
   שכל שדה טקסט אחר באפליקציה (רמזים, כיתובי ציורים, `method`) כבר עובר, ושמפרקת
   מחרוזת לפי גרשיים אחוריים ומבודדת את מה שביניהם. כל הצעדים הקיימים באפליקציה
   מעולם לא שמו גרשיים אחוריים בתוך `label` (מספרים תמיד ישבו בשדה `math` הנפרד),
   אז הבאג היה סמוי עד ששני הצעדים החדשים של הפיצ'ר הזה (הראשונים ששמים גרשיים
   אחוריים בתוך `label`) חשפו אותו — הגרשיים הוצגו כתווים ממש במקום להיעלם. תוקן ב-
   `Practice.tsx` (שורה `~582`): `<span>{segmented(step.label)}</span>`, עקבי עם
   איך שכל שדה טקסט אחר כבר מטופל.
2. **ניסוח**: "`N` מתוך `D`" הוחלף ב-"`N/D`" בכל `20` השאלות (`hints[0]` ושני
   השלבים החדשים) — השבר עצמו כמספר אחד ("10/100", "1/10"), לא מילה+שני מספרים
   נפרדים. בוצע באמצעות `perl -pi` על הדפוס `` `(\d+)` מתוך `(\d+)` `` → `` `\1/\2` ``,
   ואומת שלא נשאר אף מופע.

`npm run bump:fix` הועלה שוב (`1.17.2` → `1.17.3`). `npm run build`/`lint` נקיים,
ריצה נקייה בודדת של `npm run test:e2e`: `230/230`. אומת חזותית מול תצוגה מקדימה
בנויה מהבראנץ' — אין גרשיים גלויים, השבר `10/100`/`1/10` קריא וישר (`LTR`) בתוך
המשפט העברי, ואין שגיאות קונסול.

## תיקון שני: הכיתוב שמעל הרצועה לא אמור להסגיר את התשובה (`2026-08-19`)
המשתמש, שוב אחרי תצוגה מקדימה: "׳10% מתוך 200 הם 20 — הצבוע ברצועה׳ מתלבט אם זה
טוב וכל כך גבוה. כי זו בעצם השורה התחתונה." בדקתי איך שאר הציורים באפליקציה כבר
מתמודדים עם זה — `geometryShape.ts`'s captions (שטח/היקף) **נמנעים בכוונה** מלהצהיר
את המספר הסופי ("השטח הוא כל מה שבפנים", לא "השטח הוא `44`"), בעוד שרצועת האחוזים
כן הצהירה אותו במפורש ("הם `20`"), ממש מעל להסבר השלב-אחר-שלב. חוסר עקביות אמיתי,
לא רק תחושה.

**התיקון** (`src/data/percentStrip.ts`, הענף הראשון של `readShape()` בלבד — התבנית
"`כמה זה X% מ-Y?`"): הכיתוב הוחלף מ-`` `X%` מתוך `Y` הם `Z` — הצבוע ברצועה `` ל-
`` `X%` מ-`Y` — הצבוע ברצועה ``, לפי אותה שיטה כמו `geometryShape.ts` — מתאר את
הציור, לא מצהיר את התוצאה. שני שיקולים שנבדקו לפני היישום:
- **נגישות**: הכיתוב הזה הוא גם ה-`aria-label` של ה-`SVG` (`percent.caption.replace(/`/g, "")`
  ב-`Practice.tsx`). הוצאת המספר לגמרי הייתה פוגעת בילד/ה שלא רואים את הרצועה —
  אבל `X%` ו-`Y` (שניהם כבר נתונים בשאלה עצמה, לא התשובה) עדיין נשארים בכיתוב,
  אז התיאור עדיין משמעותי דרך קורא מסך, רק בלי להסגיר את `Z`.
- **היקף**: שלושת הענפים האחרים של `readShape()` (הנחה/העלאה/"כמה אחוזים") **לא
  נגעתי בהם** — הם כבר מוגדרים כ-Out of Scope ב-product-spec.md ("שאלות מילוליות
  באחוזים... מחוץ להיקף"), ואי-העקביות שם היא נושא נפרד שלא נפתח כאן מחדש.

`npm run bump:fix` הועלה שוב (`1.17.3` → `1.17.4`). `npm run build`/`lint` נקיים,
ריצה נקייה בודדת של `npm run test:e2e`: `230/230`. אומת חזותית: הכיתוב מציג
"`10% מ-200 — הצבוע ברצועה`" בלי "`20`", וה-`aria-label` תואם.
