...
**סבב ו׳ (2026-09-26, כפתור ההגדרות והדיאלוג):** נבנה בדיוק לפי סעיפים
1–5 ב-"סבב ו׳: הכפתור, הדיאלוג, ושתי השורות שעוברות", בשני קבצים בלבד —
בלי סטיות בהחלטות, עם שתי הוספות קטנות שהתכנון לא נקב בהן במפורש (מצוינות
למטה).

- `src/components/TopicPicker.tsx`: נוסף `useState` (ה-state הראשון ברכיב
  הזה) ל-`settingsOpen`; נוסף `showSettings = showReadAloud || showHoldZoom`
  מעל שני הדגלים הקיימים, ששניהם לא שונו; נוסף כפתור `⚙️` כילד שלישי ב-
  `.grade-header` (עם `aria-label="הגדרות"` ו-`<span aria-hidden="true">`,
  כמו `lesson-link`); ושתי שורות ההגדרה **הועברו** מגוף המסך אל תוך דיאלוג
  שמרונדר בסוף ה-JSX תחת `settingsOpen && showSettings`. ה-JSX של שתי
  השורות הועבר מילה במילה — אותן מחרוזות, אותן מחלקות, אותם `aria-*`.
  הדיאלוג: `role="dialog"` + `aria-modal="true"` + `aria-labelledby`
  שמצביע ל-`id="settings-title"` של `<h2>הגדרות</h2>`, סגירה בכפתור
  `סגירה` (`className="secondary"`, כמו "ביטול" בדיאלוג הקיים) וגם בלחיצה
  על הרקע, עם `stopPropagation` על הכרטיס. הערת קוד ליד ה-`useState`
  מזהירה מפורשות מפני `key` משתנה על `<TopicPicker>` — זה מה שהיה סוגר את
  הדיאלוג בכל בחירה, וזה לא נראה מהקובץ הזה.
- `src/App.css`: נוספו `.settings-button`, `.settings-backdrop`,
  `.settings-dialog`, `.settings-title`, `.settings-body`,
  `.settings-actions`. שתי השורות הקיימות (`.read-aloud-setting`,
  `.hold-zoom-setting`) איבדו את `max-width: 420px` ואת `margin: 0 auto 20px`
  כמתוכנן, וההערה מעל `.hold-zoom-setting` תוקנה (היא אמרה "הארגז של שורת
  ההקראה **שמעליה**" — תיאור פריסה שכבר לא קיים).
- **שתי הוספות קטנות שלא נכתבו בתכנון, שתיהן פריסה ולא התנהגות:**
  `width: 100%` ל-`.settings-dialog` (בלעדיו `max-width` לבד היה מצמצם את
  הכרטיס לרוחב התוכן, כלומר לרוחב שורת ההקראה, ושורת הבורר הייתה נשברת
  מוקדם מהנדרש), ו-`text-align: right` על `.settings-title` (בלעדיו
  הכותרת יורשת מרכוז מ-`.topic-picker`, ולא נראית כמו כותרת של מסמך עברי).

**גרסה — במפורש לא הועלתה, וזה נבדק ולא הונח.** הסבב הזה נוגע ב-`src/`,
ולכן הכלל של שלב הפיתוח אומר "העלו גרסה" — אבל `scripts/check-version-bump.mjs`
דורש **בדיוק** `middle + 1` מול הבסיס, ולא "לפחות". `origin/main` הוא
`1.31.0` וה-PR הזה כבר ב-`1.32.0` מסבב ה׳, כך שהעלאה נוספת ל-`1.33.0`
הייתה **מפילה** את הבדיקה, לא מרצה אותה. הרצתי את הסקריפט בפועל
(`version bump ok: 1.31.0 -> 1.32.0`) לפני ההכרעה.

**נבדק:** `npm run build` ו-`npm run lint` ירוקים. **הבדיקות מסבב ה׳
צפויות להיכשל כרגע ובכוונה** — שש מהן ניגשות ל-`.hold-zoom-option` ישירות
ול-`.hold-zoom-setting` כסימן שחזרנו למסך הנושאים, ושניהם עכשיו מאחורי
`⚙️`. זה שינוי ניווט בבדיקות (תפקיד שלב ה-QA), לא סימן שהמימוש שגוי; לא
"תוקן" ע"י השארת הבורר גלוי.
