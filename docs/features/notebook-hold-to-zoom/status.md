...
- [x] Tests — qa — 2026-09-26 (סבב ו׳: ארבע בדיקות חדשות, 17 בקובץ; שש בדיקות סבב ה׳ עברו לגשת דרך הכפתור; שני קבצי בדיקות אחרים דרשו תיקון ניווט — read-aloud-questions.spec.ts ו-style-lessons.spec.ts — והבדיקה של "אין קול" חוזקה במקום להותאם; 385/386 בריצה מלאה ונקייה, הכשל היחיד הוא ה-flake הקיים-מראש)
...
**Current phase:** done — מוזג ל-`main` (2026-09-26, אחרי שהמשתמש בדק את התצוגה המקדימה של סבב ו׳ ואמר "עובד טוב"). 385/386 בריצה מלאה ונקייה; הכשל היחיד הוא בדיקה קיימת-מראש שאינה של הפיצ'ר הזה — ראו Open questions.
**Branch:** `feature/notebook-hold-to-zoom`
**PR:** [#70](https://github.com/eyalzur/learn-math/pull/70) — מוזג 2026-09-26

## Open questions / blockers
**נמזג עם CI אדום — בהחלטה מודעת של המשתמש, וזה מתועד כאן כדי שלא ייראה
כהשמטה.** ה-job `check` היה ירוק; ב-`test` נכשלה **בדיקה אחת** מתוך `386`:
`tests/e2e/grade8-angles-congruence.spec.ts` › "the triangle-angle-sum step
shows the calculation with explicit brackets". אומת מדידתית שהיא **לא של
הפיצ'ר הזה**: הרצה שלה בלבד שלוש פעמים על `origin/main` נקי (worktree
ב-`a47985d`, בלי שורה אחת מהסבב) נכשלה ב-**2 מתוך 3**. הוצגה למשתמש עם
שלוש חלופות, והוא בחר "מזג עכשיו, תקן את הבדיקה בנפרד".

**תוקן בפועל, כבר לפני המיזוג הזה — לא נראה כך רק בגלל תזמון.** התיקון האמיתי
(לא רק הארכת timeout) נדחף ל-[#71](https://github.com/eyalzur/learn-math/pull/71)
ומוזג ל-`main` ב-08:27 UTC, כ-`8` דקות **לפני** שהבראנץ' הזה נסגר ותועד כאן
(08:34 UTC) — הבראנץ' של הפיצ'ר הזה נפתח לפני 2026-09-26 ולא ראה את המיזוג ההוא
לפני שהתיעוד הזה נכתב, ולכן התיאור למעלה ("התיקון הוא עבודה נפרדת") כבר לא היה
נכון ברגע שנכתב. אומת מחדש (2026-09-26, סשן נפרד): `386/386` בסוויטה המלאה על
`main` הנוכחי, ועוד `8` הרצות מבודדות, `0` כשלים. ראו
`docs/features/grade8-angles-congruence/status.md` ו-`tests.md` לאבחון המלא
ולפרטי התיקון.

**עדיין לא אומת ידנית (לא חוסם, לא נכנס לאישור):** זום-שתי-אצבעות (pinch)
תוך כדי החזקה. המשתמש אישר את התנהגות הדיאלוג ואת המחווה ("עובד טוב"), אבל
ה-pinch לא נבדק במפורש — ראו למטה.

שני דברים לא-חוסמים מסבב א׳ שעדיין רלוונטיים:
