...
שלוש חלופות, והוא בחר "מזג עכשיו, תקן את הבדיקה בנפרד". **הסיבה השורשית,
כפי שנראית מהחוץ:** הבדיקה סורקת עד `30` שאלות בחיפוש תבנית מסוימת בעוד
שתרגול הוא `20` שאלות — כשהתבנית לא יוצאת בהגרלה, הלולאה ממשיכה מעבר לסוף
התרגול, `.problem-text` כבר לא קיים, והבדיקה תולה עד timeout. **התיקון הוא
עבודה נפרדת** ורשום ב-`CLAUDE.md` תחת "מה פתוח".
