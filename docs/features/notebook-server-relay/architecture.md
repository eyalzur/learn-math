...
(כיום "זה הדף שהמורה מסתכל עליו"; במקור "זה מה שהשרת קיבל" — ראו סבב הרוויזיה
בסוף הקובץ) מומשה כ-`<h2>` (לא `<p>` כמו דיאלוג המחיקה) כי design.md
...
### Affected Files
שינוי טקסט בלבד. אין שינוי ב-state, בזרימה, ב-CSS, או בחוזה מול השרת.

- `src/components/PracticeNotebook.tsx` — שתי מחרוזות בדיאלוג התמונה
  (שורות 511-512): הכותרת ב-`<h2>` וה-`alt` של ה-`<img>`.
- `tests/e2e/notebook-server-relay.spec.ts` — שני assertions שנועלים את אותן
  שתי מחרוזות (שורות ~103, ~106). **הם ייכשלו** ברגע שהרכיב ישתנה, וזה צפוי
  ונכון: הם עושים בדיוק את מה שנועדו לו. עדכונם הוא עבודת qa.
- `package.json` — `npm run bump:fix` (בראנץ' `fix/`, והשינוי נוגע ב-`src/`).

### Technical Approach
החלפת שתי מחרוזות מילה במילה מ-design.md. אין מה להחליט מעבר לזה.

### מלכודת שהתגלתה בסריקה, ורלוונטית ישירות ל-qa

הקריטריון החדש ב-product-spec ("שום מחרוזת שהתלמיד/ה רואה אינה משתמשת במונח
תשתית") מזמין אכיפה אוטומטית בחיפוש על המילה `שרת`. **חיפוש כזה לא יעבוד:**
`שרת` הוא תת-מחרוזת של `עשרת`, ולכן `grep -rn "שרת" src/` מחזיר כ-40 תוצאות
שגויות מתוך שאלות כיתה א׳ ו-ב׳ ("עשרת אחת ועוד 4 יחידות", "שואלים עשרת אחת
מהעשרות"). בפועל יש בכל `src/` בדיוק **שתי** התאמות אמיתיות, שתיהן ב-
`PracticeNotebook.tsx`.

זו בת-דודה של מלכודת האותיות הסופיות שכבר תועדה ב-CLAUDE.md (`בלון` אינו
תת-מחרוזת של `בלונים`): **התאמת מחרוזות על עברית שבירה**, ובמקרה הזה היא נכשלת
לכיוון של יותר מדי ולא של פחות מדי. כלל תוכן שיישען על חיפוש נאיבי יהיה רועש
עד חסר-תועלת. qa צריכ/ה להכריע אם בכלל כדאי כלל כזה, ואם כן — על בסיס מה
(למשל גבולות מילה, או רשימת מחרוזות מפורשת ולא חיפוש חופשי).

### Edge Cases
אין חדשים. הדיאלוג, מצביו, והזרימה זהים לחלוטין; רק הטקסט בתוכם שונה.

### Risks / Tradeoffs
- **ה-`alt` השתנה גם הוא, וזו הכרעת designer ולא בקשה מפורשת של המשתמש** —
  מסומן ככזה ב-design.md כדי שיהיה קל לבטל אם המשתמש לא מסכים.
- **הניסוח מדבר על "המורה" בהווה** בעוד שאין עדיין קלוד. נשקל והוכרע
  ב-product-spec: "המורה" באפליקציה הזו מעולם לא היה מודל שפה אלא פרסונת
  ההוראה שלה, שכבר קיימת בפיצ'ר "מורה שמסבירה" ומחושבת כולה בדפדפן.
