...
**התחזית הייתה `15` קבצים, וזה בדיוק מה שיצא.** הארכיטקטורה ספרה `33` שימושים
ב-`.level-card` ב-`15` קבצים. באמצע העבודה נראה כמו `5`, ותוקן כאן כלפי מטה — **וזו
הייתה טעות**: הריצות שעליהן התבססה ההורדה רצו במקביל זו לזו וחלקו את אותו שרת, ולכן
הסתירו כשלים אמיתיים. אחרי ריצה בודדת ונקייה התברר שההערכה המקורית קלעה.

**הלקח אינו על הספירה אלא על המדידה:** שתי ריצות מקבילות של הסוויטה אינן שתי מדידות,
הן מדידה אחת מקולקלת.
...
- **`perfect-score-message.spec.ts`** · **`question-hints.spec.ts`** ·
  **`read-aloud-questions.spec.ts`** — כניסה, ובשניים הראשונים גם אורך התרגול.
- **`read-aloud.spec.ts`** · **`review-status.spec.ts`** · **`version-number.spec.ts`** ·
  **`teaching-explanations.spec.ts`** — לא אכפת להם דרך איזה מסך נכנסו, ולכן הם בוחרים
  `.style-card, .level-card` ולוקחים את מה שיש. בדיקה שנושאה "אפשר בכלל להתחיל
  לתרגל" לא צריכה לדעת איך בוחרים שיעור.
- **`topics-and-progress.spec.ts`** — `8` שימושים, הכי הרבה בקובץ אחד. נוסף `enterFirstLesson`
  שנכנס למה שהנושא מציג ומחזיר את שם השיעור, כי הבדיקות כאן עוסקות בהיסטוריה ובניווט.
- **`topics-all-grades.spec.ts`** · **`students-and-syllabus.spec.ts`** — טענתן היא על
  **רמות** ("שלוש רמות של עשר"), ולכן הן מכוונות עכשיו לנושא חד-סגנוני שבו עדיין יש
  רמות, במקום לנושא הראשון שכבר אין בו. הטענה נשמרה; מה שהשתנה הוא איפה היא נבדקת.
...
**וארבע בדיקות שהיו קשורות למספר שרירותי.** לכל רמה בדיוק עשר שאלות, ולשיעור יש כמה
שיש — `8` ל"מה בא אחרי", `15` ל"חיבור". ארבע בדיקות קידדו את ה-`10` הזה:

| בדיקה | מה הייתה קשורה אליו |
|---|---|
| `the conversation does not change the score` | `0 מתוך 10` |
| `the next question is reachable mid-conversation` | `שאלה 2` |
| `taking a hint does not cost anything in the score` | לולאה של `10`, ואז `10` |
| `shows the perfect-score message…` (ושתי אחיותיה) | `harvestAnswers(page, 10)` |

**כולן קוראות עכשיו את המצב מהמסך במקום לנקוב במספר** — את סך השאלות מהמונה, או את
המונה לפני ואחרי. זה מה שהן באמת בדקו מלכתחילה, והקישור ל-`10` היה מקריות של הנתונים
שהחזיקה עד היום.
