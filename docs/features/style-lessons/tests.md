...
**התחזית הייתה `15` קבצים; בפועל נגעו `10`.** הארכיטקטורה ספרה `33` שימושים
...
- **`read-aloud.spec.ts`** · **`review-status.spec.ts`** — שני אלה לא אכפת להם דרך איזה
  מסך נכנסו, ולכן הם בודקים `.style-card, .level-card` ובוחרים את מה שיש. בדיקה
  שנושאה "אפשר בכלל להתחיל לתרגל" לא צריכה לדעת איך בוחרים שיעור.
