...
**Current phase:** done — ממתין לפתיחת PR (המתאם)
...
- **QA (סבב רוויזיה א׳, 2026-09-01):** שש בדיקות חדשות ב-`tests/e2e/notebook-default-practice.spec.ts`
  (לא קובץ נפרד) על viewport נייד (`390×700`, תואם את הבאג המדווח): ברירת המחדל
  ממלאת רוב הגובה הפנוי בלי גלילה לראות את כפתור הרמז; "⤢" מגדיל את התיבה עצמה
  (לא רק זום); השאלה נשארת נגישה במלואה (טקסט זהה, לא מקוצר) דרך הפס המצומצם;
  קריאה ודאית יוצאת ממסך מלא אוטומטית; קריאה לא-ודאית נשארת בתוכו; כפתור היציאה
  חוזר למצב רגיל. נבדק גם שאין קובץ e2e אחר שהניח גובה קבוע או השתמש ב-aria-label
  הישן של הכפתור. סוויטה מלאה: `306/306`, ריצה בודדת ונקייה, ללא רגרסיה. פרטים
  מלאים ב-tests.md.
