...
- **תוקן (2026-09-26):** בדיקת ה-e2e "the triangle-angle-sum step shows the
  calculation with explicit brackets" נכשלה שוב ושוב אחרי המיזוג (עד `~50%`
  מהריצות) — לא flake של עומס אלא באג לוגי אמיתי ב-`adaptiveAngles.ts`/הבדיקה:
  ברגע ש-streak חוצה `2`, הקושי רק מטפס וזה הופך את tier `2` (ושם נמצא דפוס
  `∡C` שהבדיקה מחפשת) לבר-הישג פעם אחת בלבד לסשן — נחיתה על האח התאום
  ("הזווית החיצונית") שם הפכה את המרדף למטרה שכבר לא ניתנת להשגה. תוקן ונבדק
  ב-`100/100` ריצות מבודדות ו-`12/12` בקובץ המלא, מוזג ב-
  [#71](https://github.com/eyalzur/learn-math/pull/71) (08:27 UTC). **פרט
  תפעולי:** ה-PR נפתח ב-2026-09-13 עם תיקון-timeout ראשוני בלבד, והתיקון
  האמיתי נדחף אליו רק ב-2026-09-26 בבוקר בלי לעדכן את תיאור ה-PR — ולכן
  `docs/features/notebook-hold-to-zoom/status.md`, שנכתב `8` דקות מאוחר יותר
  על בראנץ' אחר שלא ראה את המיזוג הזה, עדיין תיעד את הבדיקה כשבורה. אומת מחדש
  בסשן הנוכחי (2026-09-26): `386/386` בסוויטה המלאה, ועוד `8` הרצות מבודדות
  (קובץ מלא + הבדיקה הבודדת), `0` כשלים. ראו tests.md לפירוט המלא של האבחון
  והתיקון.
...
- **QA (סבב ג׳, tech-lead flagged, 2026-09-05):** העוגן `"משלימות"` ב-
  ...
  אחרת. **תוקן (qa, 2026-09-05):** העוגן הוחלף ל-`"נצבים"` בשני המקומות, פלוס
  ארבע בדיקות חדשות לקריטריוני הקבלה של סבב ג׳ (דיאגרמה עם השאלה, שיעור "למה
  זה נכון", סימון חפיפה מלא, סוגריים בשלב סכום זוויות). `12/12` בקובץ, `366/366`
  בסוויטה המלאה. ראו tests.md לפירוט המלא.
