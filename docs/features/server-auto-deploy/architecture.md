...
נבנה לפי התוכנית, ללא סטיות ארכיטקטוניות. שני קבצים:
`.github/workflows/deploy-server.yml` (חדש) ו-`server/README.md` (סעיף חדש
במיקום שנקבע; הסעיף הידני נשאר שלם, כולל שני המכשולים המתועדים).

**לא הועלתה גרסה**, כמתוכנן — ה-diff אינו נוגע ב-`src/`, `public/` או
`index.html`.

### מה אומת בפועל

`npm run build` ו-`npm run lint` עברו נקי, אבל הם לא נוגעים ב-YAML ולכן אינם
עדות לכלום כאן. מה שכן נבדק:

1. **המבנה, מהמבנה המפורסר ולא ממבט בקובץ** — `name`, הטריגרים (ואי-קיום
   `paths:`), `permissions` (כולל `id-token: write`), `concurrency` עם
   `cancel-in-progress: false`, `fetch-depth: 0`, וה-`if` על כל צעד — כולם
   נקראו מ-`yaml.safe_load` והושוו למה שהארכיטקטורה קבעה.
2. **לוגיקת זיהוי השינוי, מול היסטוריית git אמיתית** — ארבעת המסלולים נבדקו על
   commits אמיתיים בריפו: commit שנגע ב-`server/` (`dbff5841`) → `changed=true`;
   commit של תיעוד בלבד (`9f5ca84a`) → `changed=false`; SHA של אפסים → פריסה;
   ו-commit שלא ניתן לפתרון → פריסה. ארבעתם התנהגו כמתוכנן. זו הלוגיקה היחידה
   כאן עם הסתעפות אמיתית, ולכן היחידה שיכולה להיות שגויה בשקט.
3. **בלוק ההכוונה זהה מילה במילה ל-design.md** — הושווה ב-`diff`, לא בעין.
4. **אין סוד בקובץ** — סריקה למפתחות פרטיים, `service_account` JSON, וגם
   ל-`secrets.*` בכלל. אין אף אחד מהם; זה מה ש-WIF קונה.

### מה **לא** ניתן לאמת, ומדוע

**ה-workflow מעולם לא רץ.** אי אפשר להריץ אותו לפני שההגדרה החד-פעמית בוצעה
בחשבון גוגל קלאוד של המשתמש, וזו פעולה שאינה בידיי. המשמעות המדויקת: ההזדהות
מול גוגל (WIF), ההרשאות, ופקודת הפריסה עצמה **לא נבדקו בפועל** — נבדקה רק
נכונותם מול התיעוד של גוגל ומול הפקודה הידנית שכן הצליחה היום.

הבדיקה האמיתית הראשונה תהיה הריצה הידנית שהמסמך מנחה את המשתמש לבצע מיד אחרי
ההקמה (**Actions → Deploy server to Cloud Run → Run workflow**). היא נבחרה
ככלי אימות בכוונה: ריצה ידנית תמיד מנסה לפרוס בפועל, ולכן היא בודקת את כל
השרשרת ולא רק את הצד שכבר עובד.

### סטייה קטנה אחת מהתוכנית

בצעד 3 של סקריפט ההקמה (הענקת חמשת ה-roles) נוסף `--condition=None`, שלא הופיע
