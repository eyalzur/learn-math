...
נבנה בדיוק לפי התוכנית, כולל שני התיקונים שתועדו מראש (טעויות ציון-הרמה של
design.md, החרגת `25%`/`÷4`).

- שתי פונקציות העזר (`wantsDecimal`, `round2`) נוספו ל-`adaptiveHelpers.ts` בדיוק
  כפי שתוכנן.
- כל שישה כיווצי הטווחים (`tenPercent`, `fiftyPercent`, `unitFraction`,
  `rectangleArea`, `square`, `averageSpeed`) בוצעו כליטרלים בלבד, ללא שינוי מבנה.
- שיפור פיתגורס: `SMALL_TRIPLES = TRIPLES.slice(0, 4)`, ו-`findHypotenuse`
  משתמש ב-`pick(SMALL_TRIPLES, rng)` ישירות במקום `scaledTriple(rng, 2)`.
- כל תשעת מנגנוני הזרעת העשרוני יושמו לפי טבלת "מחולל-מחולל" של הארכיטקטורה,
  כולל שתי החרגות ה-`25%`. בכל מקום שבו ערך מחושב (לא רק `answer` הסופי אלא גם
  ערכי `math` בשלבי הביניים, למשל `שטח והיקף`'s `product`/`answer` ו-`בעיות
  מילוליות`'s `paid ÷ remaining`) — נעטף ב-`round2()`, גם כשהניתוח הראה שהוא כבר
  מדויק, כדי לעמוד בדרישת הארכיטקטורה "כל תוצאה מחושבת עוברת `round2()`" באופן
  אחיד ולא לפי שיקול דעת נקודתי.
- **וידוא בפועל (לא רק ניתוח ידני)**: הרצתי דגימה של `300` שאלות בכל אחד מ-`18`
  השילובים הרלוונטיים (כל תשעת ה-tier המוכווצים/tier המוזרעים, פלוס בדיקת-בקרה
  על שלושת המחוללים החריגים) — `0` הפרות מתוך `5,400` דגימות: אף מספר תלת-ספרתי
  ברמת קושי `1` (או רמה `2` באחוזים), ואף תשובה עם יותר משתי ספרות אחרי הנקודה
  באף אחד מתשעת המחוללים עם הזרעת עשרוני. שיעור ההיארעות בפועל נע בין `27%`
  ל-`58%` (תלוי בכמה החרגת `25%` מצמצמת את הדגימה האפקטיבית) — סביר ועקבי עם
  היעד של `~30%`. הסקריפט הזמני נמחק בסיום, לא נשאר בריפו.
- `npm run build` ו-`npm run lint` נקיים. `npm run bump:feature` הועלה
  (`1.18.0` → `1.19.0`, בהמשך ל-`levels-as-practice` שכבר על הבראנץ' הזה).
- לא הרצתי את סוויטת ה-e2e המלאה — זו עבודת `qa`. `content.spec.ts` כבר מכיל
  (מ-`levels-as-practice`) בדיקה שדוגמת את כל `12` המחוללים מול `RULES` — היא
  אמורה לתפוס כל בעיית דליפת-רמז/חשבון-שגוי שהניתוח הידני והדגימה שלי פספסו.
