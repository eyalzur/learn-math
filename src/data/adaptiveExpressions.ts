...
function combineLikeTerms(rng: Rng): Question {
  ...
  return makeQuestion(
    ...
    answer,
    [
      "אוספים יחד רק איברים עם אותו משתנה: המקדם הוא המספר שכופל את המשתנה",
      `מסתכלים רק על המקדמים \`${a}\` ו-\`${b}\`, ו${isAdd ? "מחברים" : "מחסרים"} ביניהם — בלי לגעת ב-\`${v}\``,
    ],
    [
      { label: `${isAdd ? "מחברים" : "מחסרים"} רק את המקדמים של אותו המשתנה (אצלנו \`${v}\`)` },
      { label: "", math: `${a} ${op} ${b} = ${answer}` },
    ],
    isAdd
...
function openBrackets(rng: Rng): Question {
  ...
  return makeQuestion(
    ...
    answer,
    [
      "כופלים את מה שמחוץ לסוגריים בכל אחד מהאיברים שבפנים",
      `\`${a}\` הוא המקדם (המספר שכופל את \`${v}\`), ו-\`${a}\` כפול \`${b}\` הוא המספר החופשי`,
    ],
    [
      { label: "כופלים את המספר שמחוץ לסוגריים בכל איבר בסוגריים" },
      { label: "כופלים את המשתנה:", math: `${a} × ${v} = ${a}${v}` },
      { label: "כופלים את המספר:", math: `${a} × ${b} = ${answer}` },
      { label: "הביטוי הפתוח:", math: `${a}${v} + ${answer}` },
    ],
